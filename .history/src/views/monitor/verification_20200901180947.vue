<template>
  <div class="verification">
    <div v-show="searchshow">
      ...
      <div class="filter-container">
        勾选来源：
        <el-checkbox label="小红书" border />
        <el-checkbox label="陌陌" border />
        <el-checkbox label="在行一点" border />
        <el-checkbox label="有道" border />
        <el-checkbox label="悟空问答" border />
        <el-checkbox label="趣头条" border />
        <!-- <el-checkbox v-for="item in applist"  v-model="listQuery.app[item.id]" :label="item.name" border></el-checkbox> -->
      </div>
      <div class="filter-container">
        勾选类型：
        <el-checkbox label="图文类" border />
        <el-checkbox label="社交类" border />
        <el-checkbox label="新闻类" border />
        <el-checkbox label="问答类" border />
        <el-checkbox label="笔记类" border />
        <!-- <el-checkbox v-for="item in applist"  v-model="listQuery.app[item.id]" :label="item.name" border></el-checkbox> -->
      </div>
      <div class="filter-container">
        关键词
        <el-input v-model="listQuery.keyword" style="width: 300px;" class="filter-item" />
    ...
    >
      ...
      <el-table-column label="APP名称" width="110px" align="center">
        <template slot-scope="{row}">
      ...
      <el-table-column label="处置源头" class-name="status-col" width="100">
        <template slot-scope="{row}">
...
export default {
  ...
  methods: {
    ...
    handleArticle(row, para) {
      this.listLoading = true
      disposeArticle(this.baseUrl, { id: row.id, status: para }).then((response) => {
        console.log('response:')
        console.log(response)
        setTimeout(() => {
          this.$message({
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['app名称', '标题', '内容', '发布时间', '发布账号', '是否有害']
        const filterVal = ['name', 'title', 'content', 'timestamp', 'author', 'status']
